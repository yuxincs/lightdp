...
def main():
    ...
    jobs = [
        ...
        {
            "algorithm": sparse_vector_v2,
            "kwargs": {'T': 10.5, 'N': 1},
            "D1": lambda eps: [10 for _ in range(10)],
            "D2": lambda eps: [9 for _ in range(10)],
            "S": lambda: [1]
        },
        {
            "algorithm": sparse_vector_v3,
            "kwargs": {'T': -100, 'N': 1},
            "D1": lambda eps: [10 for _ in range(6)],
            "D2": lambda eps: [9 for _ in range(6)],
            "S": lambda: fisher_s_selector(algorithm, (), kwargs, D1, D2, epsilon,
                                           search_space=[[i] for i in range(7)])
        },
        {
    ...
    for job in jobs:
        results.clear()
        if job['algorithm'].__name__ == 'sparse_vector_v1':
            ...
            for algorithm_epsilon in [0.2, 0.5, 0.7] + list(range(1, 4)):
                ...
                for epsilon in [x / 10.0 for x in range(1, 34, 1)]:
                    ...
                    draw_graph('Test $\epsilon$', 'P Value', results,
                               ('auto '+algorithm.__name__.replace('_', ' ')).title(),
                               'auto '+algorithm.__name__ + '.pdf')
